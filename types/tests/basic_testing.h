...
#ifdef BT_MACOS_NO_WRAP
/* On macOS, the linker doesn't support --wrap, so we disable memory instrumentation */
static int bt_mem_checks_disabled_macos = 1;
#undef bt_mem_checks_disabled
#define bt_mem_checks_disabled bt_mem_checks_disabled_macos
#endif
...
#ifdef BT_MACOS_NO_WRAP
    /* reallocarray is not available on all macOS versions, use realloc instead */
    if (nmemb == 0 || size == 0 || SIZE_MAX / nmemb <= size)
	return NULL;
    size_t len = nmemb * size;
